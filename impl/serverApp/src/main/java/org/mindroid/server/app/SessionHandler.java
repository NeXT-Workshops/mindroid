...
public class SessionHandler {
    ...
    private SessionHandler() {
        LOGGER = LogManager.getLogger(SessionHandler.class);
    }
    ...
    public void handleSessionMessage(MindroidMessage msg) throws IOException {
        ...
        if(sessionCommand == MindroidMessage.BAD_SESSION_SIZE){
            ...
            switch (currentState) {
                case IDLE:
                    switch (sessionCommand) {
                        case MindroidMessage.QUIT_SESSION:
                            ...
                            LOGGER.warn(robot + " tried to leave session, while SessionHandler is IDLE");
                            break;
                        case MindroidMessage.UNCOUPLED_SESSION:
                            ...
                            LOGGER.info(robot + " started uncoupled Session");
                            break;
                        case MindroidMessage.START_SESSION:
                            // should not occur, do nothing
                            LOGGER.warn(robot + " tried to start session");
                        default: // s>0
                    ...
                    switch (sessionCommand) {
                        ...
                        default:
                            ...
                            if (sessionCommand == maxSessionSize) {
                                ...
                                sessionRobots.add(robot);
                                LOGGER.info(robot + " joins Session");

                                if (sessionRobots.size() == maxSessionSize) {
                                    LOGGER.info("Session is full, start RUNNING_COUPLED");
                                    ...
                                    LOGGER.info("Session not full yet, keep PENDING");
                                    currentState = SessionState.PENDING;
                                    sendSessionUpdate(sessionRobots.size());
                                }
                            } else {
                                LOGGER.warn(robot + " tried to join Session but sent wrong size");
                ...
                case RUNNING_COUPLED:
                    if (sessionCommand == MindroidMessage.QUIT_SESSION) {
                        ...
                        sessionRobots.clear();
                        currentState = SessionState.IDLE;
                        LOGGER.info(robot + " leaves Session, going to IDLE");
                        msf.addLocalContentLine("INFO", "Robot quit session, session ended");
                    } else {
                        LOGGER.warn(robot + " tried to join, but RUNNING_COUPLED");
                ...
                case RUNNING_UNCOUPLED:
                    switch (sessionCommand) {
                        case MindroidMessage.QUIT_SESSION:
                            LOGGER.info(robot + " leaving RUNNING_UNCOUPLED");
                            sessionRobots.remove(robot);
                            if (sessionRobots.isEmpty()){
                                LOGGER.info("RUNNING_UNCOUPLED is empty, going to IDLE");
                                currentState = SessionState.IDLE;
                            }else{
                                LOGGER.info("RUNNING_UNCOUPLED not empty yet, keep RUNNING UNCOUPLED");
                                currentState = SessionState.RUNNING_UNCOUPLED;
                            }
                            break;
                        case MindroidMessage.UNCOUPLED_SESSION:
                            LOGGER.info(robot + " joins RUNNING_UNCOUPLED");
                            sessionRobots.add(robot);
                            msw.sendMessage(startSessionMessage, robot);
                            break;
                        default:
            ...
            updateSessionLabel();
            LOGGER.info("State after: " + currentState);
        }
    }

    private void sendSessionUpdate(int sessionUpdate){
        LOGGER.info("Sending update of Session, int: " + sessionUpdate);
